...
export async function clearSelectionByClickingEmptyCanvas(
  page: Page,
  preferredWorldPoint: Point = { x: 20, y: 20 }
): Promise<void> {
  const canvas = page.locator('#game-container canvas');
  await expect(canvas).toBeVisible();
  ...
  for (const p of candidates) {
    ...
    await clickCanvasAt(page, p);
    const selection = await page.evaluate(() => (window as any).__PHASER_FORGE_TEST__?.getState?.()?.selection ?? null);
    if (selection?.kind === 'none') return;
  }

  throw new Error('Failed to clear selection by clicking empty canvas');
}
