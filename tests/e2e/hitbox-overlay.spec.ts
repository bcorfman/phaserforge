...
test('shows a labeled hitbox overlay and supports toggling @browser', async ({ page }) => {
  ...
  const e1 = await page.evaluate(() => window.__PHASER_FORGE_TEST__?.getEntityWorldRect('e1') ?? null);
  ...
  await expect.poll(async () => {
    return await page.evaluate(() => window.__PHASER_FORGE_TEST__?.getHitboxOverlayInfo?.() ?? null);
  }).toMatchObject({ visible: true });

  const before = await page.evaluate(() => window.__PHASER_FORGE_TEST__?.getHitboxOverlayInfo?.() ?? null);
  ...
  await expect.poll(async () => {
    const next = await page.evaluate(() => window.__PHASER_FORGE_TEST__?.getHitboxOverlayInfo?.() ?? null);
